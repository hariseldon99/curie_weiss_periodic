    ...
    def __init__(self, params):
        ...
        self.hamiltmat = self.jx * np.sum(np.array(\
          [self.jmat[sitepair] * self.kemats(sitepair, memmap=params.memmap)[0] \
            for sitepair in combinations(range(self.lattice_size),2)]), axis=0)
        ...
        self.trans_hamilt = self.hx * np.sum(np.array([self.nummats(mu, memmap=params.memmap)[0] \
                                             for mu in range(self.lattice_size)]), axis=0)
...
def run_dyn(params, initstate=None):
    ...
    sx = np.sum(np.array([h.nummats(mu)[0] for mu in range(lsize)]), axis=0)
    sy = np.sum(np.array([h.nummats(mu)[1] for mu in range(lsize)]), axis=0)
    sz = np.sum(np.array([h.nummats(mu)[2] for mu in range(lsize)]), axis=0)
